...
export class CodeBuildDeploymentStack extends cdk.Stack {
  ...
  constructor(scope: Construct, id: string, props: CodeBuildDeploymentStackProps) {
    ...
    const logGroup = new logs.LogGroup(this, 'BuildLogGroup', {
      logGroupName: `/aws/codebuild/hotline-qa-${envName}`,
    ...
    const buildRole = new iam.Role(this, 'CodeBuildRole', {
      ...
      description: 'IAM role for Hotline QA CodeBuild project',
      managedPolicies: [
    ...
    this.buildProject = new codebuild.Project(this, 'DeploymentProject', {
      projectName: `hotline-qa-deployment-${envName}`,
      description: `Automated deployment for Hotline QA - ${envName} environment`,
        ...
        phases: {
          ...
          'pre_build': {
            commands: [
              ...
              'echo "BUCKET_PREFIX=${BUCKET_PREFIX:-hotline-qa}"',
          ...
          build: {
            commands: [
              ...
              './deploy.sh --env ${ENV_NAME:-dev} --bucket-prefix ${BUCKET_PREFIX:-hotline-qa} --github-owner ${GITHUB_OWNER} --github-repo ${GITHUB_REPO} --github-token-secret ${GITHUB_TOKEN_SECRET}'
            ]
          },
          'post_build': {
            commands: [
