...
function extractPartialJSON(jsonString: string): LLMOutput {
  ...
  const fieldPatterns = [
    /"Tone"\s*:\s*\{[^}]*\}/g,
    /"Professional"\s*:\s*\{[^}]*\}/g,
    /"Active Listening"\s*:\s*\{[^}]*\}/g,
    /"Initial Assurance"\s*:\s*\{[^}]*\}/g,
    /"Problem Exploration"\s*:\s*\{[^}]*\}/g,
    /"Risk Assessment"\s*:\s*\{[^}]*\}/g,
    /"Collaborative Planning"\s*:\s*\{[^}]*\}/g,
    /"Call Closure"\s*:\s*\{[^}]*\}/g
  ];
...
function createFallbackStructure(): LLMOutput {
  ...
  const allCriteria = [
    ...CATEGORIES.RAPPORT_CONNECTION,
    ...CATEGORIES.COUNSELING_PROCESS_SAFETY
  ];
...
export const handler = async (event: StepFunctionsEvent): Promise<StepFunctionsEvent> => {
  ...
  try {
    ...
    const fieldMappings: {[key: string]: string} = {
      'Empathetic Tone': 'Tone',
      'Professionalism': 'Professional',
      'Conversational Style': 'Active Listening',
      'Supportive Initial Statement': 'Initial Assurance',
      'Explores Problem(s)': 'Problem Exploration',
      'Clarifies Non-Suicidal Safety': 'Risk Assessment',
      'Restates then Collaborates Options': 'Collaborative Planning',
      'Appropriate Termination': 'Call Closure'
    };
...
function aggregateScores(llmOutput: LLMOutput): AggregatedScores {
  const result: AggregatedScores = {
    ...
    totalPossibleScore: 32, // Total possible score is 32 (8 criteria * 4 multiplier)
    percentageScore: 0,
    criteria: ""
  };
  ...
  // Determine criteria based on actual score out of 32
  // Calculate thresholds: 80% and 70% of total possible score
  const meetsThreshold = result.totalPossibleScore * 0.80; // 25.6 out of 32
  const improvementThreshold = result.totalPossibleScore * 0.70; // 22.4 out of 32
